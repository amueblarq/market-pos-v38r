    ...
    static public function mdlRegistrarPerfil($perfil)
{
    ...
    try {
        // Verificar si la descripción del perfil ya existe
        $stmt_check = $dbh->prepare("SELECT COUNT(*) AS count FROM perfiles WHERE descripcion = ?");
        $stmt_check->execute([$perfil['descripcion']]);
        $result_check = $stmt_check->fetch(PDO::FETCH_ASSOC);

        if ($result_check['count'] > 0) {
            // Si la descripción del perfil ya existe, no se permite el registro
            $respuesta['tipo_msj'] = 'error';
            $respuesta['msj'] = 'La descripción del perfil ya existe';
            return $respuesta;
        }

        // Si la descripción del perfil no existe, se procede con el registro
