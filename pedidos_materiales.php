...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_verificar_pedido = "SELECT COUNT(*) as total_pedidos FROM pedidos WHERE codigo_orden = '$codigo_orden'";
    ...
    if ($resultado_verificar && $fila_verificar = $resultado_verificar->fetch_assoc()) {
        ...
        if ($total_pedidos > 0) {
            ...
            $sql_pedidos = "INSERT INTO pedidos (codigo_orden, categoria, descripcion, cantidad_materiales) 
                            VALUES ('$codigo_orden', '$categoria', '$descripcion', $cantidad_materiales)";

            if ($conexion->query($sql_pedidos) === TRUE) {
...
?>
        <form id="pedidoForm" action="procesar_pedido.php" method="POST" onsubmit="return validarCampos()">
